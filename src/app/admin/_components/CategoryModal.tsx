        ...
        <div className="bg-[#00000066] fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center ">  
            <div className=" bg-white rounded-lg lg:w-[460px] lg:h-[272px]">
                      <div className="flex justify-between p-8">
                            ...
                            <button onClick={()=> onClose(false)}><MdCancel className="size-8" /></button>
                      </div>
                      <div className="pl-8 pr-8 text-end">
                        <div className="text-start font-normal ">
                          <h3>Category name</h3>
                          <Input className="opacity-85 mt-2" value={newCategory} onChange={(e) => {setNewCategory(e.target.value)}} placeholder="Type category name..."/>
                        </div>
                        <button className="bg-black text-white p-2 mt-8 font-normal rounded-lg text-sm" onClick={saveButton} >Add category</button>
                      </div>
